...
export async function getSubtitles(options: {
  ...
  // Select track: exact match, then regional variant (de → de-DE),
  // then English (en/en-*), else first available
  const langLower = lang.toLowerCase();
  const matchesBaseLang = (code: string, base: string) => {
    const c = code.toLowerCase();
    return c === base || c.startsWith(base + '-');
  };
  let track = playerTracks.find(t => t.languageCode.toLowerCase() === langLower)
    || playerTracks.find(t => matchesBaseLang(t.languageCode, langLower));
  if (!track) {
    if (!enableFallback) {
      throw new Error(`Language '${lang}' not available. Available: ${playerTracks.map(t => t.languageCode).join(', ')}`);
    }
    track = playerTracks.find(t => matchesBaseLang(t.languageCode, 'en')) || playerTracks[0];
  ...
  // Drop fmt to get plain XML; handle query params position-independently
  const trackUrl = new URL(track.baseUrl);
  trackUrl.searchParams.delete('fmt');

  // Caption URLs carrying exp=xpe are gated behind a proof-of-origin token
  const expFlags = trackUrl.searchParams.get('exp');
  if (expFlags && expFlags.split(',').includes('xpe')) {
  ...
  try {
    ...
    lines = parseTimedTextXml(xml);
  } catch (err) {
    throw new TranscriptFetchError(`Failed to fetch transcript: ${(err as Error).message}`, isLive, metadata);
  }
